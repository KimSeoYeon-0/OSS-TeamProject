...
robot.settings(200, 200, 100, 100)  # 속도 및 가속도 설정
...
def shoot(command):
    if command == 'zero':
        shooting_motor.run_until_stalled(-150, Stop.COAST, duty_limit=70) 
        shooting_motor.reset_angle(0)
    elif command == 'shoot':
        shooting_motor.run(3500)  # 더 높은 속도와 각도로 슈팅 설정
        time.sleep(0.25)
        shooting_motor.stop()

def grab(command):
    if command == 'motion3':  # 공을 잡기
        grab_motor.run_target(1000, 0, Stop.COAST) # hold도 써봐
        grab_motor.reset_angle(0)
    elif command == 'motion1':  # 팔 열기
        grab_motor.run_target(1000, -150, Stop.COAST)
    elif command == 'motion2':  # 팔 위치 조정
        grab_motor.run_target(1000, -150, Stop.COAST)

def turn_to_angle(target_angle, speed=90):
    """
...
def shoot_towards_opponent():
    ...
    turn_to_angle(current_angle + angle_difference, speed=60)  # 최소 회전만 수행
